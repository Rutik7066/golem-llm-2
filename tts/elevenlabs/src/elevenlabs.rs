...
use golem_tts::{
    ...
    config::get_env,
    golem::tts::{
        advanced::{AgeCategory, AudioSample, PronunciationLexicon},
        streaming::{SynthesisStream, VoiceConversionStream},
        synthesis::{SynthesisResult, ValidationResult},
        types::{SynthesisMetadata, TextType, TtsError, VoiceGender},
        voices::{LanguageInfo, Voice, VoiceFilter, VoiceResults},
    },
...
use crate::{
    ...
    resources::{
        ElPronunciationLexicon, ElVoice, ElevenLabsVoiceConversionStream, ElevenLabsVoiceResults,
    },
    types::{
        AddVoiceResponse, CreateLexiconFromRulesRequest, CreateLexiconResponse, ElVoices,
        ListVoicesQuery, PronunciationRule, PvcCreateRequest, PvcCreateResponse,
...
impl TtsClient for Elevenlabs {
    ...
    ) -> Result<golem_tts::golem::tts::streaming::SynthesisStream, TtsError> {
    ...
    fn create_voice_conversion_stream(
        target_voice: String,
        options: Option<golem_tts::golem::tts::streaming::SynthesisOptions>,
    ) -> Result<golem_tts::golem::tts::streaming::VoiceConversionStream, TtsError> {
        Ok(VoiceConversionStream::new(
            ElevenLabsVoiceConversionStream::new(target_voice, options),
        ))
    }
    ...
    fn synthesize(
        ...
        voice: String,
        options: Option<golem_tts::golem::tts::streaming::SynthesisOptions>,
    ) -> Result<golem_tts::golem::tts::synthesis::SynthesisResult, TtsError> {
    ...
    fn synthesize_batch(
        ...
        voice: String,
        options: Option<golem_tts::golem::tts::streaming::SynthesisOptions>,
    ) -> Result<Vec<golem_tts::golem::tts::synthesis::SynthesisResult>, TtsError> {
        ...
        for (index, input) in inputs.iter().enumerate() {
            ...
            let body = SynthesisRequest {
                ...
                pronunciation_dictionary_locators: None,
                seed: options.as_ref().and_then(|o| o.seed.map(|s| s as i32)),
                previous_text: inputs.get(index - 1).map(|i| i.content.clone()),
    ...
    fn get_timing_marks(
        ...
        input: golem_tts::golem::tts::streaming::TextInput,
        voice: String,
    ) -> Result<Vec<golem_tts::golem::tts::streaming::TimingInfo>, TtsError> {
    ...
    fn get_voice(&self, voice_id: String) -> Result<Voice, TtsError> {
        let voice = self.client.retry_request::<ElVoice, _, (), _>(
            Method::GET,
    ...
    ) -> Result<golem_tts::golem::tts::advanced::PronunciationLexicon, TtsError> {
