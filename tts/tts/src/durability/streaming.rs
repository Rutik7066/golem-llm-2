...
mod durable_impl {
    ...
    impl<Impl: ExtendedGuest> DurableSynthesisStream<Impl> {
        fn live(stream: Impl::SynthesisStream) -> Self {
            Self {
                state: RefCell::new(Some(DurableSynthesisStreamState::Live { stream })),
            }
        }
    ...
    impl<Impl: ExtendedGuest> GuestSynthesisStream for DurableSynthesisStream<Impl> {
        ...
        fn close(&self) {
            let state = self.state.borrow();
            if let Some(DurableSynthesisStreamState::Live { stream }) = &*state {
                stream.close();
            }
        }
    ...
    impl<Impl: ExtendedGuest> StreamingGuest for DurableTTS<Impl> {
        ...
        fn create_stream(
            ...
            if durability.is_live() {
                ...
                match inner_stream {
                    Ok(stream) => Ok(SynthesisStream::new(DurableSynthesisStream::<Impl>::live(
                        stream,
                    ))),
                    Err(e) => Err(e),
                }
