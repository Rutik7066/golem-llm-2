...
use crate::golem::tts::advanced::{
    AudioSample, LanguageCode, LongFormOperation, PronunciationEntry, PronunciationLexicon,
    VoiceDesignParams,
};
...
impl ApiClient {
        ...
        match request.send() {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    trace!("Received response from API: {}", text.clone());
        ...
        F: Fn(Response) -> TtsError,
    {
        ...
        // Use auth_headers only (already set as default headers in client)
